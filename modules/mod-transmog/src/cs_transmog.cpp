...
public:
    ...
    static bool HandleAddTransmogItem(ChatHandler* handler, Optional<PlayerIdentifier> player, ItemTemplate const* itemTemplate)
    {
        ...
        if (sTransmogrification->AddCollectedAppearance(accountId, itemId))
        {
            ...
            {
                ChatHandler(target->GetSession()).PSendSysMessage(R"(|c{}|Hitem:{}:0:0:0:0:0:0:0:0|h[{}]|h|r has been added to your appearance collection.)", itemQuality, itemId, itemName);
            }

            // Feedback of successful command execution to GM
            if (isNotConsole && target != handler->GetPlayer())
            {
                handler->PSendSysMessage(R"(|c{}|Hitem:{}:0:0:0:0:0:0:0:0|h[{}]|h|r has been added to the appearance collection of Player {}.)", itemQuality, itemId, itemName, nameLink);
            }
        ...
        else
        {
            ...
            if (isNotConsole)
            {
                handler->PSendSysMessage(R"(Player {} already has item |c{}|Hitem:{}:0:0:0:0:0:0:0:0|h[{}]|h|r in the appearance collection.)", nameLink, itemQuality, itemId, itemName);
                handler->SetSentErrorMessage(true);
            }
        }
    ...
    static bool HandleAddTransmogItemSet(ChatHandler* handler, Optional<PlayerIdentifier> player, Variant<Hyperlink<itemset>, uint32> itemSetId)
    {
        ...
        if (isNotConsole)
        {
            ...
            if (!added)
            {
                handler->PSendSysMessage("Player {} already has ItemSet |cffffffff|Hitemset:{}|h[{} {}]|h|r in the appearance collection.", nameLink, uint32(itemSetId), setName, localeNames[locale]);
                handler->SetSentErrorMessage(true);
                return true;
            }
            ...
            {
                handler->PSendSysMessage("ItemSet |cffffffff|Hitemset:{}|h[{} {}]|h|r has been added to the appearance collection of Player {}.", uint32(itemSetId), setName, localeNames[locale], nameLink);
            }
        }
        ...
        {
            ChatHandler(target->GetSession()).PSendSysMessage("ItemSet |cffffffff|Hitemset:{}|h[{} {}]|h|r has been added to your appearance collection.", uint32(itemSetId), setName, localeNames[locale]);
        }
